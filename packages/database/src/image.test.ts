...
import { createDatabase, migrateDatabase, ImageGenerationSettingsRepository, SceneImageGenerationRepository, WorkflowRepository } from './index.js';
...
describe('scene image persistence', () => {
  ...
  it('commits a guarded generated asset and derives stale freshness', () => {
    ...
    const stepId = workflow.createStep(executionId, 'scene-image', 'GENERATE_SCENE_IMAGE', sceneRevisionId, 'generation-fingerprint', 3, {});
  ...
  it('keeps manual replacements, reviews, current pointers, and project scope coherent', () => {
    ...
    expect(repository.updateReview(projectId, first.id, { status: 'ACCEPTED', notes: 'good' }))
      .toMatchObject({ reviewStatus: 'ACCEPTED', notes: 'good' });
    ...
    expect(repository.list(projectId, sceneStableId).map((image) => image.revision)).toEqual([2, 1]);
    expect(() =>
      repository.setCurrent(projectId, sceneStableId, second.id, 2),
    ).toThrow(/changed/);
    database.sqlite.close();
  });
});

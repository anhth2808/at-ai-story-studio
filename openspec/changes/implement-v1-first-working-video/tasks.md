...
- [ ] 10.3 Run typecheck, lint/format checks, automated tests, and the actual API/web/worker smoke path with SQLite, filesystem, Edge TTS, FFmpeg, and ffprobe; verify create project/chapter, multi-segment narration, subtitle generation, background upload, MP4 playback, restart persistence, failed-segment-only retry, and chapter-scoped invalidation.
- [ ] 10.4 Record executed commands/results, environment limitations, deviations from `docs/design-v1`, known limitations, exact local run commands, manual test procedure, and the recommended next milestone without implementing it; verify the final report does not claim unperformed real integrations.

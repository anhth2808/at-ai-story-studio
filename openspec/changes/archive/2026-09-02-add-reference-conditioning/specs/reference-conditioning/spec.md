...
The system SHALL let a user generate and view, for one Scene, both a TEXT_ONLY and a REFERENCE_CONDITIONED image and compare them side by side with generation metadata (mode, workflow, seed, references used). Regeneration SHALL support same-seed and new-seed conditioned generation, and technical retry of a conditioned generation SHALL reuse the same generation, fingerprint, seed, and reference mapping.

#### Scenario: Compare baseline and conditioned
- **WHEN** a Scene has one completed TEXT_ONLY and one completed REFERENCE_CONDITIONED generation
- **THEN** the UI SHALL present both images side by side with their persisted metadata without loading image binaries into metadata APIs

### Requirement: Real conditioned benchmark verification
This capability SHALL NOT be considered complete until, against a real configured ComfyUI server: one recurring character with one approved reference has been generated across at least five Scenes with scene-varied composition, each Scene generated both conditioned and text-only under controlled settings; results have been manually reviewed and scored for face identity, hair, clothing, style, and prompt adherence; the multi-character behavior has been tested or honestly documented as limited; and a LoRA decision (`LORA_REQUIRED_NOW` YES or NO) has been recorded with the observed evidence.

#### Scenario: Complete the benchmark
- **WHEN** the milestone verification is performed
- **THEN** evidence SHALL list the real generation records and assets for both modes across at least five Scenes, manual consistency scores, observed improvements and failures, the multi-character verdict, and the recorded LoRA decision

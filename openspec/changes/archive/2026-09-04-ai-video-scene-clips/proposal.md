...
- Add `FakeVideoProvider` for deterministic tests, scale simulation (100 chapters x 5 scenes, 70/20/10 mode mix) without GPU runs, and focused tests for request mapping, workflow validation, fingerprints, staleness, reuse, hybrid duration composition, fallback, retry-vs-regenerate, and restart state.

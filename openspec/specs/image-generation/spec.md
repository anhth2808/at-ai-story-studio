...
The system SHALL create image-generation requests from one persisted `CURRENT` Visual Prompt Package and SHALL carry its project, Scene, package identity, effective positive prompt, negative prompt, fingerprint, requested dimensions, concrete seed, bounded generation settings, and reference-asset identifiers. An image provider SHALL NOT independently query or reconstruct Story Blueprint, chapter prose, characters, locations, StoryState, or other narrative context.
...
The system SHALL expose a bounded image-provider contract for generation, status/result retrieval, classified errors, timeout, and cancellation where supported. Provider-specific workflow nodes, checkpoint files, scheduler details, and server routes SHALL remain outside provider-neutral Scene, Style Bible, and Visual Prompt Package data.

#### Scenario: Preserve reference assets for a future workflow
- **WHEN** a Visual Prompt Package contains project-owned reference asset identifiers and the selected workflow does not consume references
...
The initial ComfyUI integration SHALL execute only the application-approved `text-to-image-v1` API-format template. Prompt, negative prompt, seed, width, height, steps, guidance, sampler, configured model components, and output node mappings SHALL be validated before submission. Arbitrary workflow JSON or executable/custom-node code SHALL NOT be accepted from API clients.

#### Scenario: Expected node is missing
- **WHEN** an expected mapped node or input is removed from the approved workflow
- **THEN** generation SHALL fail with `WORKFLOW_INVALID` before the workflow is submitted

#### Scenario: Map deterministic inputs
- **WHEN** a known package and settings fixture is mapped
- **THEN** every supported prompt, seed, resolution, step, guidance, sampler, and model field SHALL appear at the configured template node/input
...
The image-generation fingerprint SHALL include the Visual Prompt Package fingerprint, provider, approved workflow/template version, output-affecting settings, model components, and concrete seed. Completion SHALL compare the original fingerprint and package freshness against current inputs before publishing. A stale completed output MAY be retained as historical evidence but SHALL NOT silently become current.
...
The initial workflow SHALL be text-to-image and SHALL NOT require LoRA, DreamBooth, IP-Adapter, InstantID, FaceID, ControlNet, pose/depth control, regional prompting, reference-image generation, vision scoring, automatic regeneration/selection, AI video, image-to-video, animation, parallax, lip sync, or a generic workflow/node editor.
...
- **THEN** the UI and documentation SHALL NOT claim pixel-perfect recurring character identity and SHALL retain reference inputs only as a future-compatible contract
